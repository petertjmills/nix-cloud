# Got from [here](https://github.com/JayRovacsek/nix-config/blob/54aa375e35f542f8bbad3811e6322f1f412ada78/options/modules/headscale/default.nix#L48C1-L62C18)
{
  config,
  lib,
  pkgs,
...
let
  ...
  script = with pkgs; ''
    bash -c "
    ${pkgs.sqlite-interactive}/bin/sqlite3 ${cfg.settings.database.sqlite.path} <<'END_SQL'
...
in
{
  ...
  config = lib.mkIf cfg.use-declarative-users {
    systemd.services.headscale-user-setup = {
      ...
      # Add restart triggers for when the configuration changes
      restartTriggers = [
        (builtins.toJSON cfg.users)
        sql-statement
      ];

      # Make it run on system activation
      wantedBy = [ "multi-user.target" ];
      # make sure we perform actions after to headscale starting
      ...
      serviceConfig = {
        ...
        Type = "oneshot";
        RemainAfterExit = true; # This helps with restart behavior
      };
    };
  };
}
